    ...
    parse: function(input) {
        ...
        for(var cur = 0; cur < fixed.length; ++cur) {
                ...
                if(end <= fixed.length && fixed.substring(start, end) == pattern.find) {
                    ...
                    if(typeof pattern.rules !== 'undefined') {
                        for(var j = 0; j < pattern.rules.length; ++j) {
                            ...
                            if(replace) {
                                output.regex += rule.replace + "(্[যবম])?(্?)([ঃঁ]?)";
                                output.patterns.push(rule.replace);
                                output.tokens.push(pattern.find);
                                cur = end - 1;
                                matched = true;
                                break;
                            }
                    ...
                    // Default
                    output.regex += pattern.replace + "(্[যবম])?(্?)([ঃঁ]?)";
                    output.patterns.push(pattern.replace);
                    output.tokens.push(pattern.find);
                    cur = end - 1;
                    matched = true;
                    break;
            ...
            if(!matched) {
                ...
                output.patterns.push(fixed.charAt(cur));
                output.tokens.push(fixed.charAt(cur));
            }
        }

        //No need with V8
        // output.regex = this._convertToUnicodeValue(output.regex);
